---
name: review-behavioral-interview
...
1. Read `AGENTS.md`, `CLAUDE.md` if it exists, the relevant group manifest (`data/<group>/index.json`), and the target `interview.json` files.
...
6. Write findings in `REVIEW.md` in the folder the reviewed interview (the folder where interview.json file is) when the user asks for a documented review, catalog audit, or reusable review process.
...
After changing only `REVIEW.md` or skill files, no build is required. After changing interview data, also run:
